...
export default function Quiz({ title='Quiz', questions=[] }) {
  ...
  const shuffled = useMemo(() => validQuestions.slice(), [validQuestions]);
    ...
    return (
      <div className="rounded-2xl border p-4 bg-white shadow-sm">
        <div className="text-center py-8">
          <p className="text-sm text-gray-600">No valid questions available.</p>
        </div>
      </div>
    );
  }

  const pct = Math.round((score / validQuestions.length) * 100);
  const pass = pct >= 70;
  ...
  return (
    <div className="rounded-2xl border p-4 bg-white shadow-sm">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-xs text-gray-500">{i + 1} / {validQuestions.length}</span>
      </div>
      {!done ? (
        <div>
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600">
            Score: {score} / {validQuestions.length} — {pass ? 'Pass' : 'Try again'} (70%)
          </p>
          <button 
            className="mt-3 px-3 py-2 rounded-xl border shadow-sm hover:bg-gray-50" 
            onClick={() => {
              setI(0);
              setSel(null);
              setScore(0);
