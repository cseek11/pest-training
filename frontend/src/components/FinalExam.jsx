...
export default function FinalExam({ questions=[], minutes=60 }){
  // randomize
  const shuffled = useMemo(()=>questions.slice().sort(()=>Math.random()-0.5),[questions])
  ...
  if(!questions.length) return <p className="text-sm text-gray-600">No final questions.</p>
      ...
      {!done ? (
        <div>
          ...
          <div className="mt-3 flex justify-end">
            <button onClick={next} className="px-3 py-2 rounded-xl border shadow-sm hover:bg-gray-50">Next</button>
          </div>
        </div>
      ) : (
        <div className="text-center">
          <div className="text-3xl font-bold">{pct}%</div>
          <p className="text-sm text-gray-600">Score: {score} / {shuffled.length} — {pass?'Pass':'Try again'} (70%)</p>
        </div>
      )}
    </div>
  )
